    ...
    public void execute() throws MojoExecutionException {
    	try {
			...
			if (new File(com.tibco.bw.maven.plugin.utils.FileUtilsProject.getApplicationMetaInf(projectBasedir).getAbsolutePath() + "/TIBCO.xml").exists()) {
				...
				if (prop.size() > 0) {
					boolean success= outputDirectory.isDirectory();
					if (!success) {
					success = outputDirectory.mkdirs();
					}
					if (success) {
					getLog().info("bwresourceExport: Create the directory Sucess. "+outputDirectory.getAbsolutePath());
					com.tibco.bw.maven.plugin.utils.FileUtilsProject.setSavePropertyOrder(prop,
							outputDirectory + "/" + propertyfile);
					}
					else
					{
						getLog().error("bwresourceExport: Error create the directory: "+outputDirectory.getAbsolutePath());
					}
				}
				else
				{
